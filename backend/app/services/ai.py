...
۲. درصد رو فقط روی عدد واقعی کاربر حساب کن:
   - اگه درآمد دقیق ماهانه ثبت شده، با همون عدد حساب کن.
   - اگه فقط بازه درآمد ثبت شده (مثل «بیشتر از ۸۰ میلیون»)، بازه رو اعلام کن و از کاربر بخواه عدد دقیق بگه. مثال: «درآمدت بالای ۸۰ میلیونه — عدد دقیق رو بهم بگو تا دقیق حساب کنم.»
   - هرگز یه عدد تخمینی رو در پرانتز یا به هر شکلی به عنوان واقعیت ذکر نکن (مثلاً «یعنی ۱۰٬۰۰۰٬۰۰۰ تومان» یا «یعنی ۱۰۰٬۰۰۰٬۰۰۰ تومان»). این کار کاملاً ممنوع است.
   - تا وقتی عدد دقیق نداری، فقط با درصد ادامه بده («۱۰٪ از درآمدت») بدون نوشتن هیچ عدد مطلقی.
...
۵. اگه کاربر بازه درآمد «بیشتر از ۸۰ میلیون» انتخاب کرده، عدد واقعی‌اش می‌تونه صدها میلیون یا حتی میلیاردها تومان باشه. هیچ‌وقت برای این بازه عدد مشخصی مثل «۱۰۰ میلیون» یا «۱۲۰ میلیون» فرض نکن — فقط بازه رو تأیید کن و عدد دقیق بخواه."""
...
نمونه ۱:
کاربر: «چطور ۲۰ درصد از حقوقم سیو کنم؟»
جواب درست (بدون JSON):
«۲۰٪ پس‌انداز انتخاب هوشمندیه! اول بهم بگو درآمد ماهانه‌ات دقیقاً چقدره تا بتونم حساب کنم.»
...
def build_system_prompt(context: Optional[dict] = None, chat_mode: str = "normal") -> str:
    ...
    if not context:
        return "\n\n".join([BASE_PERSONA, tone, BEHAVIOR_RULES, ACTION_RULES, ACTION_SPEC])
    ...
    if income_range and income_range not in ("prefer_not",):
        label = income_range_label(income_range)
        user_ctx_parts.append(f"- بازه درآمد ماهانه کاربر: {label}. عدد دقیق ثبت نشده — برای محاسبه درصد باید از کاربر بپرسی.")
    else:
        user_ctx_parts.append("- درآمد ماهانه: اعلام نشده — برای محاسبه درصد باید از کاربر بپرسی.")
    ...
    user_context_block = "\n".join(user_ctx_parts)
    return "\n\n".join([BASE_PERSONA, tone, user_context_block, BEHAVIOR_RULES, ACTION_RULES, ACTION_SPEC])

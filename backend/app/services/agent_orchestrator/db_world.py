...
WORLD_INSTRUCTIONS = [
    ...
    # Conversation-scoped deletion — uses provenance, NOT time heuristics
    "CONVERSATION-SCOPED DELETION: Every transaction and future_commitment created via chat carries a source_message_id linking it to the user chat_message that triggered its creation. When the chat history is cleared, that link is set to NULL. So 'this conversation' means WHERE source_message_id IS NOT NULL. Do NOT approximate this with created_at, today's date, or the last N rows — those may pick up manually created rows or older conversations.",
    "For 'delete everything I told you about in this conversation' style requests: DELETE FROM transactions WHERE source_message_id IS NOT NULL (and, if the user also asked about commitments: DELETE FROM future_commitments WHERE source_message_id IS NOT NULL). This tool call MUST set bulk_scope=true on the plan step. Report the actual number the executor deleted; do not claim success unless the executor reports rows deleted.",
    "For 'delete everything I told you about since [X]' style requests: first SELECT id, source_message_id FROM transactions WHERE source_message_id IS NOT NULL to see the conversation-created rows, then filter by other columns using AND, then DELETE by the resolved id list.",
    ...
    "BULK DELETION SAFETY: A DELETE that filters by anything OTHER than a specific id or a specific id list (e.g. filters by type, date, source_message_id, amount, description) is a BULK delete. You MUST set the plan step field bulk_scope=true for these. If you do not set bulk_scope=true, the executor rejects the delete unless exactly one row matches the filter — this prevents singular requests like 'delete the 500,000 toman purchase' from silently deleting several similar rows when the match is ambiguous.",
    "AMBIGUOUS SINGULAR DELETE: If the user's phrasing is singular ('delete the restaurant expense', 'حذف کن اون خرید') do NOT set bulk_scope=true. Do a SELECT first; if it returns more than one candidate, present them briefly and ask which one instead of guessing.",
